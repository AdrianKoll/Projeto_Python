...
def mostrar_calendario():
    ...
    for mes in range(1, 13):
        ...
        for semana in cal_mes:
            cal_texto += " ".join(f"{dia:>3}" if dia != 0 else "   " for dia in semana) + "\n"
        cal_texto += "\n"
        cal_text[mes - 1] = cal_texto

    cal_labels = [cal_label1, cal_label2, cal_label3, cal_label4, cal_label5, cal_label6,
                  cal_label7, cal_label8, cal_label9, cal_label10, cal_label11, cal_label12]

    for i in range(12):
        cal_labels[i].config(text=cal_text[i])
...
cal_labels = []
for i in range(12):
    cal_label = tk.Label(frame_calendario, text="", font=('Consolas', 8, 'bold'), bg="#C0C0C0",
                         width=28, height=10, anchor="n")
    cal_label.grid(row=i // 4, column=i % 4, padx=5, pady=5)
    cal_labels.append(cal_label)

cal_label1, cal_label2, cal_label3, cal_label4, cal_label5, cal_label6, \
    cal_label7, cal_label8, cal_label9, cal_label10, cal_label11, cal_label12 = cal_labels
